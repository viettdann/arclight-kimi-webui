  ...
  return (
    ...
    >
      <div className="mx-auto max-w-3xl pointer-events-auto">
        <div
          ref={cardRef}
          className="rounded-lg border border-border/70 bg-card/95 backdrop-blur supports-[backdrop-filter]:bg-card/85 overflow-hidden shadow-lg ring-1 ring-warning/10 flex flex-col max-h-[80dvh]"
        >
          <div className="px-4 py-3 flex items-start gap-3 border-b border-border/40 shrink-0">
          ...
          <div className="flex-1 overflow-y-auto min-h-0 px-4 py-2 space-y-2">
            {headline && (
              <div className="text-xs text-muted-foreground break-words">{headline}</div>
            )}
            {command && (
              <pre className="font-mono text-xs whitespace-pre-wrap break-words bg-muted/15 text-foreground/90 max-h-32 overflow-y-auto leading-relaxed px-2 py-2 rounded">
          ...
          <div className="px-3 py-2 flex flex-col sm:flex-row gap-1 sm:justify-end border-t border-border/40 shrink-0">
            <Button
              type="button"
              variant="ghost"
              size="sm"
